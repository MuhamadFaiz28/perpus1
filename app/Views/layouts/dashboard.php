...
<div class="container py-5">
    ...
    <div class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5 g-4 mb-5">
        <?php if (!empty($buku)): ?>
            ...
            ?>
                <div class="col book-item">
                    <div class="cover-wrapper mb-3 shadow-hover">
                        <div class="position-absolute top-0 start-0 m-2 d-flex flex-column gap-1" style="z-index: 10;">
                            ...
                            <?php if (session()->get('role') == 'admin'): ?>
                                <a href="<?= base_url('dashboard/delete/' . $id_key) ?>" 
                                   class="btn btn-danger btn-sm rounded-circle shadow border-white border-2 p-0 d-flex align-items-center justify-content-center" 
                                   style="width: 28px; height: 28px;"
        ...
        <?php else: ?>
            <div class="col-12 text-center py-5">
                ...
                <p class="text-muted small">Belum ada koleksi buku.</p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="card custom-table-card shadow-sm">
        <div class="card-header bg-white p-4 border-0 d-flex justify-content-between align-items-center">
            <h5 class="fw-bold m-0 text-dark"><i class="fas fa-history me-2 text-primary"></i>Sirkulasi Terbaru</h5>
            <a href="<?= base_url('peminjaman'); ?>" class="btn btn-outline-primary btn-sm rounded-pill px-4 fw-bold">Semua Data</a>
        </div>
        <div class="table-responsive p-4 pt-0">
            <table class="table align-middle">
                <thead class="bg-light">
                    <tr>
                        <th class="ps-3 border-0">Peminjam</th>
                        ...
                        <tr>
                            <td class="ps-3">
                                <div class="d-flex align-items-center">
                                    <div class="rounded-circle bg-primary bg-opacity-10 p-2 me-2 text-primary" style="width: 35px; height: 35px; display: flex; align-items: center; justify-content: center;">
                                        <i class="fas fa-user fa-xs"></i>
                                    </div>
                                    <span class="fw-bold text-dark"><?= esc($p['nama_peminjam'] ?? 'User'); ?></span>
                                </div>
                            </td>
                            <td><span class="text-muted text-truncate d-inline-block" style="max-width: 200px;"><?= esc($p['judul'] ?? '-'); ?></span></td>
                            <td>
                                <span class="fw-medium <?= (strtotime($p['tanggal_kembali']) < strtotime(date('Y-m-d')) && $p['status'] == 'dipinjam') ? 'text-danger' : '' ?>">
                            ...
                            <td class="text-end pe-3">
                                <span class="badge-status <?= $p['status'] == 'dipinjam' ? 'bg-warning text-warning' : 'bg-success text-success'; ?> bg-opacity-10">
                                    <i class="fas <?= $p['status'] == 'dipinjam' ? 'fa-hourglass-half' : 'fa-check' ?> me-1"></i>
                                    <?= ucfirst($p['status']); ?>
                                </span>
                            </td>
                        </tr>
